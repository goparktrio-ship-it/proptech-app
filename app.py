...
def run_capital_gains_tax_app():
    ...
    # 🚨 [UI 변경] 하단 설명창 내용을 셀렉트박스 ①~⑥번과 완벽하게 매칭하여 직관성 극대화!
    with st.expander("🔍 내 아파트, 살 때 규제지역이었을까? (셀렉트박스 번호별 상세 지역 및 연혁)"):
        st.markdown("""
        ...
        **🎯 핀셋 규제란?**<br>
        구 단위로 묶을 때 외곽의 읍/면/동이 억울하게 묶이는 것을 방지하기 위해 규제에서 **명시적으로 제외해 준 동네**입니다. (예: 남양주 화도읍/수동면, 파주 문산읍, 용인 처인구 포곡읍 등)<br>
        *(Tip) 우리 동네가 핀셋 규제로 제외된 곳이라면, 입력창의 '핀셋 규제 제외' 체크박스를 누르시면 앱이 알아서 예외 처리(비규제 취급) 해 드립니다.*
        """, unsafe_allow_html=True)

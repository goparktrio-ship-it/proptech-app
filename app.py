...
def run_capital_gains_tax_app():
    ...
    with col2:
        ...
        # 🚨 [UI 변경 1] 주택 수 설명창을 셀렉트박스 바로 밑으로 전진 배치! (인지 흐름 최적화)
    ...
    # 🚨 [UI 변경 2] 하단 비과세 가이드라인을 아주 간결하게 압축하여 가독성 극대화!
    with st.expander("🔍 1세대 1주택 & 일시적 2주택 비과세 요건 (핵심 요약)"):
        ...
        **⏳ 일시적 2주택 비과세 (갈아타기)**
        * **1년 텀:** 기존 주택 취득 후 **최소 1년 지나서** 신규 주택 취득
        * **3년 내 처분:** 신규 주택 취득일로부터 **3년 이내** 기존 주택 매도 (규제지역 무관)
        * **기존 주택 요건:** 매도하는 기존 주택은 '1세대 1주택 비과세 요건(2년 보유 등)'을 충족해야 함
        """, unsafe_allow_html=True)
